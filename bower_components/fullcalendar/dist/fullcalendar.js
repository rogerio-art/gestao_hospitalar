...
DayGrid.mixin({
	...
	renderSegRow: function(row, rowSegs) {
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			emptyCelltil(colCnt); // finish off the row
...
DayGrid.mixin({
	...
	limitRow: function(row, levelLimit) {
		...
		function emptyCelltil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				seg = levelSegs[i];
				emptyCelltil(seg.leftCol); // process empty cells before the segment
			...
			emptyCelltil(this.colCnt); // finish off the level
...
var View = FC.View = ChronoComponent.extend({
	...
	requestEventrender: function() {
		var _this = this;

		this.renderQueue.queue(function() {
			_this.executeEventrender();
		}, 'event', 'destroy');
	},
	...
	executeDateUnrender: function() {
		...
		this.trigger('before:daterendered');
	...
	bindBaseRenderHandlers: function() {
		...
		this.on('before:daterendered.baseHandler', function() {
	...
	handleDocumentMousedown: function(ev) {
		if (isPrimaryMouseButton(ev)) {
			this.processelect(ev);
		}
	},


	processelect: function(ev) {
...
View.mixin({
	...
	initHiddenDays: function() {
		...
		if (this.opt('weekends') === false) {
			hiddenDays.push(0, 6); // 0=day, 6=saturday
		}
