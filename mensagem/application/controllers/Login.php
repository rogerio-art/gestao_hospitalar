...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('Bem Vindo').' '.$row->first_name.' '.$row->last_name);
            if ($row->role_id == 1) {
                $this->session->set_userdata('admin_login', '1');
                redirect(site_url('admin/message'), 'refresh');
            }else if($row->role_id == 2){
                $this->session->set_userdata('user_login', '1');
                redirect(site_url('/home/my_messages'), 'refresh');
            }
        }else {
            $this->session->set_flashdata('error_message',get_phrase('invalid_login_credentials'));
            redirect(site_url('home/home'), 'refresh');
